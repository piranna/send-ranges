...
module.exports = (fetchStream, opts = {}) => {
  ...
  return (() => {
    var _ref = _asyncToGenerator(function* (req, res, next) {
      ...
      var _file = file;
      const getStream = _file.getStream,
            size = _file.size,
            type = _file.type,
      ...
      const intersectRanges = options.intersectRanges;
      ...
      if (isUnsatisfiable) {
        res.status(416).set('Content-Range', `bytes */${size}`).type('text').send('Range not satisfiable');
        return;
      }

      // Beyond limit?
      if (ranges.length > options.maxRanges) {
        res.status(400).set('Content-Range', `bytes */${size}`).type('text').send(`Too many ranges specified. Max: ${options.maxRanges}. Range: ${range}`);
        return;
      }
      ...
      const resolveStream = isSingleRange ? getStream(ranges[0]) : new BRS({ range, getChunk: getStream, totalSize: size, contentType: type });

      // Stream retrieval might be async
      const sourceStream = yield resolveStream;
      ...
      options.beforeSend(info, function (err) {
        if (err) {
          next(err);
          return;
        }
